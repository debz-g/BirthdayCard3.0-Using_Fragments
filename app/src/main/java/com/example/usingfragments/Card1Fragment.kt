...
class Card1Fragment : Fragment(R.layout.fragment_card1) {
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val vi=inflater.inflate(R.layout.fragment_card1,container,false)

        val btnShareOne=vi.findViewById<Button>(R.id.btnShare1)
        val cardOne=vi.findViewById<ImageView>(R.id.ivCard1)
        btnShareOne.setOnClickListener {
            // Step 1: Create Share itent
            val intent = Intent(Intent.ACTION_SEND).setType("image/*")

            // Step 2: Get Bitmap Drawable file from imageView
            val bitmapDrawable = cardOne.drawable as BitmapDrawable // your imageView here.
